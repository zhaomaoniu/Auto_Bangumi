...
import { ErrorPicture, Play } from '@icon-park/vue-next'; // 替换 Write 为 Play
...
<template>
    <div v-if="type === 'primary'" w-150 is-btn @click="() => $emit('click')">
        <div rounded-4 overflow-hidden poster-shandow rel>
            ...
            <div abs f-cer z-1 inset-0 opacity-0 transition="all duration-300"
                hover="backdrop-blur-2 bg-white bg-opacity-30 opacity-100" active="duration-0 bg-opacity-60"
                class="group">
                <div text-white rounded="1/2" wh-44 f-cer bg-theme-row group-active="poster-pen-active">
                    <Play size="20" /> <!-- 替换 Write 为 Play -->
                </div>
            </div>
        </div>
    ...
    <div v-else-if="type === 'search'" w-480 rounded-12 p-4 shadow bg="#eee5f4"
        ...
        <div bg-white rounded-8 p-12 fx-cer justify-between gap-x-16>
            <div w-400 gap-x-16 fx-cer>
                <div h-44 w-72 rounded-6 overflow-hidden>
                    <template v-if="bangumi.poster_link">
                        <img :src="bangumi.poster_link" alt="poster" w-full translate-y="-25%" />
                    </template>
                ...
                <div flex="~ col gap-y-4">
                    ...
                    <div flex="~ gap-x-8">
                        <template v-for="i in ['season', 'group_name', 'subtitle']" :key="i">
                            <ab-tag v-if="bangumi[i]" :title="i === 'season' ? `Season ${bangumi[i]}` : bangumi[i]"
                                type="primary" />
                        </template>
                    </div>
                </div>
